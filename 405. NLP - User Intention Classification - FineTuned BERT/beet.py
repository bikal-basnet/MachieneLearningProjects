...
class EmbeddingIntentClassifier(Component):
    ...
    def _create_encoded_intents(self, intent_dict):
        ...
        self.pprint(' CREATING ENCODED INTENTS')
        if self.intent_tokenization_flag:
            ...
            print('\n1. Intent token dictionary is ', intent_token_dict)
            print('\n 2. Encoded all Intent  is ', encoded_all_intents )

            for key, idx in intent_dict.items():
                for t in key.split(self.intent_split_symbol):
                    encoded_all_intents[idx, intent_token_dict[t]] = 1

            print('\n3. Final encoded_intent is ', encoded_all_intents)
    ...
    def _prepare_data_for_training(self, training_data, intent_dict):
        ...
        print(np.stack([e
                      for e in training_data.responses]))
        ...
        self.pprint('PREPARE DATA FOR TRAINING')
        print("\n0. X np.stacked for all text_ffeatures is  ", X)
        print("\n1. intents_for_X   ", intents_for_X)
        print("\n1. Y is ", Y)
