...
class Environment(object):
    ...
    def create_agent(self, agent_class, *args, **kwargs):
        ...
        print('agent states is ',self.agent_states[agent])
        return agent
    ...
    # After every step, update the computer controlled dummy agents, the traffic light
    def step(self):
    ...
    #Get the new changed environment information.
    ...
    def act(self, agent, action):
        ...
        state = self.agent_states[agent]
        # object is a referenced, hence the object is dynamically updated at the runtime and we will getting the new state
        #state_prior_to_action = self.agent_states[agent]
        location = state['location']
        heading = state['heading']
        # get_state returns
        state_prior_to_action = agent.get_state()  # {location, heading}

        light = 'green' if (self.intersections[location].state and heading[1] != 0) or ((not self.intersections[location].state) and heading[0] != 0) else 'red'
        prev_heading = ''
        # Prev heading, priot to taking action
        # valid_headings = [(1, 0), (0, -1), (-1, 0), (0, 1)]  # ENWS
        if (heading == (0, 1)):
            prev_heading = 'South'
        if (heading == (1, 0)):
        ...
        if action == 'forward':
            ...
            if light == 'green':
                # if move is not allowed by the light rules and right of way rule, then change the heading
                #  and take the action i.e  if action is left, but the light is
                #print('---- Action is  left and light is green. Hence changing the heading (Car face) from ', heading, 'to', heading[1], -heading[0] )
                #time.sleep(10)
                heading = (heading[1], -heading[0])
            else:
                move_okay = False
        elif action == 'right':
            # if move is allowed by the light rules and right of way rule, then change the heading
            #print('---- Action is right. Hence changing the heading (Car Face ) from ', heading, 'to', -heading[1],  heading[0])
            #time.sleep(10)
            heading = (-heading[1], heading[0])

        if action is not None:
            if move_okay:
                # if move is ok, valid, then update the location of the agent now
                ...
                ## location and the heading  updated, but the agent's
                # if action is allowed by the  traffic  light and other env factors, then we add some reward
                reward = 2 if action == agent.get_next_waypoint() else 0.5
            else: # if  action is not valid, then we give it the reward of -1, so that our learner can learn based  on these negative rewards,
                reward = -1
        else:
            reward = 1
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                if state['deadline'] >= 0:
                    reward += 10  # bonus
                self.done = True
                print('/************************************************* ENDS : Destination Reached *********************************************************************************/')
                print "Environment.act(): Primary agent has reached destination!"  # [debug]
            #    print('get state is', self.state['heading'])

            self.status_text = "Previous State : {}  Previously Car was Facing : {} \n Now Car is  Facing   {} after taking action :   " \
                               "{}. \n Reward earned was : {} Current state after action is : \n{} "\
                .format(state_prior_to_action,  prev_heading, heading,  action, reward, state )
...
class DummyAgent(Agent):
    ...
    def update(self, t):
        ...
        action_okay = True
        if self.next_waypoint == 'right': # we want to go right next,  the light is red,  and there is car coming from left moving forward then we cannot make the right turn
            if inputs['light'] == 'red' and inputs['left'] == 'forward':
                action_okay = False
        elif self.next_waypoint == 'forward': # we want to go forward but the light is red, then action should not be taken
            if inputs['light'] == 'red':
                action_okay = False
        elif self.next_waypoint == 'left': # we want to go left
