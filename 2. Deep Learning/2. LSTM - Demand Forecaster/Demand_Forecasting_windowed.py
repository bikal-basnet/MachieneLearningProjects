...
def test_train_data_splitter(dataset, train_data_size_in_percentage = 0.67):
    train_size = int(len(dataset) * train_data_size_in_percentage)
...
def generate_predictions_iteratively(use_ts_model, for_data, look_back = 1):
    #print('for_data is ', for_data)
    #for_data  = numpy.around(for_data, decimals=3 )
    #prediction = use_ts_model.predict(for_data, batch_size = 4)
    # print('prediction is', 'prediction', type(prediction), prediction.shape\
    #          , 'for value', for_data,for_data.shape, len(for_data))
    #sys.exit(1)
    # print('prediction for data is ', type(for_data), for_data.shape)
    data = numpy.ndarray(shape=(1,1,look_back), dtype=float)
    for i in range(0, look_back):
      #print('i',i)
      data[0][0][i] = for_data[i][0][0]
    # data1 = numpy.array([[[ for_data[0][0][0], for_data[1][0][0],\
    #                       for_data[2][0][0] , for_data[3][0][0] ]]])
    #print('data.shape is ', data.shape, data.shape)
    all_predictions = []
    #sys.exit()
    all_predictions = numpy.empty([len(for_data), 1], dtype=float)
    index = 0
    #for i in range(0, 2):
    for i in range(0, len(for_data) - 1):
        data = numpy.around(data, decimals=3)
        # print for_data[i][0][0]
        # print('prediction CURRENT data is ', type(data), data.shape, data)
        #data = numpy.array([[[for_data[i][0][0]]]])
        prediction = use_ts_model.predict(data)
        if i+look_back < len(for_data):
          print('PREDICTION is', numpy.around(prediction[0][0], decimals= 3), ' vs ',\
                numpy.around(for_data[i+look_back][0][0], decimals= 3) , 'for  input data', data)
        all_predictions[i] = prediction[0][0]
        # print('prediction was done on the old data. Old data was', data)
        # print('excluded from window shift is',data[0][0][0])
        for j in range(0, look_back):
          if j != look_back - 1:
            data[0][0][j] = data[0][0][j+1]
          else:
            #print('populate ', j, '  with  ', prediction[0][0])
            data[0][0][j] = prediction[0][0]

          # data[0][0][1] = data[0][0][2]
          # data[0][0][2] = data[0][0][3]

        #print('after pred input data  is',data)
        # data = numpy.array([[[ for_data[index+1][0][0], for_data[index+2][0][0] ,\
        #                      for_data[index+3][0][0], prediction[0][0] ]]])
        # #print('new data to predict on is',data)
        index = index + 1
        #sys.exit()
    # expected output is numpy.ndarray(60,1)
    #print('all prediction is ', 'all_predictions', type(all_predictions), all_predictions.shape )
    #sys.exit()
    return all_predictions


def generate_predictions(use_ts_model, for_data):
     prediction = use_ts_model.predict(for_data)
     #print('prediction is', prediction, type(prediction), prediction.shape, 'for value', for_data.shape, len(for_data))
     # sys.exit(1)
     return prediction
...
if __name__ == "__main__":
    ...
    inputfilepath = 'Data/Skygeek.csv'
    ...
    train, test = test_train_data_splitter(dataset, 0.90)
    ...
    look_back = 4
    trainX, trainY = convert_data_to_TS(train_new, look_back=look_back)
    testX, testY = convert_data_to_TS(test , look_back=look_back)
    model = get_ts_model(trainX, trainY, look_back, nb_epochs=100)
    trainPredict = generate_predictions(model, trainX)

    testPredict = generate_predictions_iteratively(model, testX, look_back=look_back)
